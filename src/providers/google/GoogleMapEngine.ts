...
function buildSvgIcon(config: SvgIconConfig): google.maps.Symbol {
  return {
    ...
    anchor: config.anchor
      ? new google.maps.Point(config.anchor.x, config.anchor.y)
      : null,
  };
}
...
export class GoogleMapEngine extends MapEngine {
  ...
  constructor(options: MapEngineOptions) {
    ...
    this.liveController = new LiveMotionController(
      this.markerAdapter,
      this.options.liveMotionPolicy,
    );
  ...
  async mount(element: string | HTMLElement): Promise<google.maps.Map> {
    const apiKey =
      this.options.apiKey || (import.meta as any).env.VITE_GOOGLE_MAPS_API_KEY;
    if (!apiKey) {
  ...
  protected onThemeChange(theme: ThemeName): void {
    ...
    if (this.map) {
      ...
      console.log(
        `[GoogleMapEngine] Applying styles: ${styleCount}, bg: ${backgroundColor}`,
      );
      this.map.setOptions({ styles, backgroundColor });
      this.map.getDiv().style.backgroundColor = backgroundColor;
    } else {
      console.warn(
        "[GoogleMapEngine] Map not initialized, skipping theme update",
      );
    }
  }

  private adaptToInput(
    vehicle: VehicleLike,
    lat: number,
    lng: number,
    id: string | number,
    ...
    if (vehicle.icon) {
      ...
      markerOptions.icon = {
        ...
        scaledSize: iconConfig.size
          ? new this.googleApi!.maps.Size(
              iconConfig.size[0],
              iconConfig.size[1],
            )
          : null,
        anchor: iconConfig.anchor
          ? new this.googleApi!.maps.Point(
              iconConfig.anchor[0],
              iconConfig.anchor[1],
            )
          : null,
    ...
    if (!isNaN(lat) && !isNaN(lng)) {
      if (this.options.infoWindowRenderer) {
        existing.infoWindow.setContent(
          this.options.infoWindowRenderer(vehicle),
        );
      }

...
export class TripReplayController {
  ...
  load(coordinates: any[]): void {
    ...
    if (this.coordinates.length >= 2) {
      ...
      if (first.ts && last.ts) {
        ...
        // Support alternative timestamp key
        this.baseDuration =
  ...
  play(
    options: { speed?: number; duration?: number } = {},
    onFinish?: () => void,
  ): void {
    ...
    const rawPath = this.coordinates
      .map((c) => {
    ...
    if (this.animationPath.length > 0) {
      ...
      const start =
        firstSeg.type === "move" ? firstSeg.start : firstSeg.position;
  ...
  private startAnimationLoop(): void {
    const animate = (time: number) => {
      ...
      const segment = this.animationPath.find(
  ...
  private prepareAnimationPath(rawPath: any[], totalDuration: number) {
    ...
    const totalMoveDist = segments
      .filter((s) => s.type === "move")
      .reduce((acc, s) => acc + s.distance, 0);
    const totalStopDur = segments
